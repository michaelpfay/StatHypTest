...
# an update to simulateSS is now available in the asht R package
# this old version is kept so that it can match the results in the book (at least with R version 4.1.2)
simulateSS<-function(decFunc,dataGenFunc,nstart=100,numBatches=100,repsPerBatch=100,power=0.30, 
 ...
 # estimate power from first batch
  ...
  for (i in 5:numBatches){
    ...
    # in original, the code is the following 
    if (!upDown && diff(N[(i-nrepeatSwitch+1):i])==rep(0,nrepeatSwitch-1)) upDown<-TRUE
    # 
    # in asht the code is changed to....
...
Nest<-rep(NA,10)
for (i in 1:10){
  Nest[i]<-simulateSS(decFunc,dataGenFunc,nstart=100,numBatches=100,repsPerBatch=100,power=0.80, alpha=0.025,printSteps=FALSE)$Nestimate
}
table(Nest)
power.t.test(delta=.5,sig.level=0.025,power=.8,type="two.sample",alternative="one.sided")
set.seed(301)
s<-simulateSS(decFunc,dataGenFunc,nstart=100,numBatches=100,repsPerBatch=1000,power=0.80, alpha=0.025,printSteps=TRUE)
s$Nstar
s$Nestimate


simulateSS(decFunc,dataGenFunc,nstart=60,numBatches=50,repsPerBatch=100,power=0.80, alpha=0.025,printSteps=TRUE)