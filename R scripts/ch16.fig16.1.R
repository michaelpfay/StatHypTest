...
CItypes<-(c("Greenwood with log transformation","Modified Lower","Beta Product Confidence Procedure","Borkowf","Strawderman-Wells","Thomas-Grunkemeier"))
...
GW<-list(time=gw$time,lower=gw$lower,upper=gw$upper,surv=gw$surv)
class(GW)<-"kmci"
...
lines(bfit,linetype="ci",col=gray(CIgray[3]),lwd=CILWD[3],lty=CILtypes[3])
...
#DIR<-"S:/BRB/Staff Folders/Mike Fay/book/ASHT/R"
#dev.print(pdf, file="S:/BRB/Staff Folders/Mike Fay/book/ASHT/book/graph/chCenNWTCO.pdf")